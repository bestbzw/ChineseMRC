...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        # 如果没有给出数据地址的话，通过tensorflow_datasets来读取。
        ...
        #这部分不知道可不可以复用
        features, dataset = lic2020_convert_examples_to_features(
            examples=examples,
            tokenizer=tokenizer,
...
def main():
    ...
    for path in model_name_or_path:
        args.model_name_or_path = path
        args.output_result_dir = path
        logging.info("checkpoint now {}".format(args.model_name_or_path))
        build_model_and_eval(args)
